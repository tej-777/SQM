...
const HowItWorks = () => {
  ...
  const steps = [
    {
      ...
      description: "Choose your hospital and department easily from the list."
    },
    {
      icon: (
        <svg className="w-8 h-8 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      icon: (
        <svg className="w-8 h-8 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
            d="M15 17h5l-1.4-1.4A2 2 0 0118 14v-3" />
        </svg>
      ),
      title: "Get Notified",
  ...
  return (
    <section
      id="How-It-Works"
      className="relative bg-blue-200 flex items-center justify-center min-h-screen [mask-image:linear-gradient(to_top,transparent,black_15%,black_80%,transparent)] "
    >
      ...
      <div className="max-w-8xl ">
        ...
        <div className="flex flex-row gap-16 justify-between items-center">
          {steps.map((step, index) => (
            <motion.div
              key={index}
              className="relative w-86 h-72 rounded-2xl border border-gray-100 bg-white shadow-sm hover:shadow-md transition flex flex-col items-center justify-center"
              whileHover={{ y: -4 }}
            >
              {/* Step Number */}
